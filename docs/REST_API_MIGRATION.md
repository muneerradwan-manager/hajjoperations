...
> **آخر تحديث:** 2026-08-12 — **البلاغ يُشطَب، والغرفة تنتبه** (migration
...
- تعديل تقرير عبر `save_report` يظهر سطراً واحداً على `reports` مع
  ...
  لا تُسجَّل إطلاقاً.
- **فخّ يستحقّ النصّ (0121):** 0077 علّقت الـ trigger على كل جدولٍ كان موجوداً
  **يوم نفاذها**، وتركت ملاحظةً بأن أي جدول يُنشأ بعدها يبدأ **غير مسجَّل** ما
  لم تعلّقه هجرته بنفسها. ولم تقرأ 0088 الملاحظة: `incidents` و
  `incident_attachments` بقيا خارج السجلّ من يوم إنشائهما حتى 0121. وأُضيفت
  معهما حالة `incidents` إلى `record_label` — صفّ البلاغ لا يحمل `name` ولا
  `title` ولا أياً من الأعمدة الستّة التي ترجع إليها الدالّة، فكان سطره يُكتب
  **معرّفاً عارياً**؛ صار أول ٨٠ حرفاً من `body`. أي backend بديل: **التسجيل
  افتراضٌ لا اشتراك**، وجدولٌ جديد يخرج منه هو خطأ لا خيار.
