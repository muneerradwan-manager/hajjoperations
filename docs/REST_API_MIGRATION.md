...
> **آخر تحديث:** 2026-08-07 — **سجلّي أنا** (لا migration ولا صلاحية): §30.3
...
على الخادم مع **مطابقة عربية مطوَّعة** (همزات/تاء مربوطة موحّدة — دالة `ar_fold`
تطبَّق على الطرفين).
...
| صلاحية صاحبها | كاملة: تعديل، حذف، حالة، مرفقات | **تغيير الحالة فقط** (مع الملاحظة والأدلة) |
| صلاحية كاتبها | — (هو صاحبها) | كاملة، وكذلك المدير |

**الجدول:** `id`, `profile_id` (صاحب القائمة), `created_by` (الكاتب),
`title` (لغة واحدة — قائمة خاصة لا أوراق إدارة), `description?`, `due_on?`,
`state` (`not_started`|`in_progress`|`done` — enum أعيدت تسميته
`module_task_state` → `task_state`), `note?`, `sort_order`, `created_at`,
`updated_at`, `updated_by` (يختمهما trigger عند كل تعديل).
...
```
GET    /me/tasks                    → قائمتي كاملة (ما كتبتُه وما أُسند إليّ) مع المرفقات، الأحدث أولاً
GET    /me/assigned-tasks           → ما كتبتُه على قوائم الآخرين (للمتابعة)، مع اسم صاحب كل قائمة
                                      — يتطلب tasks.assign
...
POST   /tasks                       → { profile_ids?: [uuid], title, description?, due_on? }
                                      ...
                                      **قائمة لا معرّفاً واحداً**: المهمة الواحدة تُسلَّم لستة
                                      مشرفين بقرار واحد، فتُكتب **صفوفهم في statement واحد**
                                      — ستّ رحلات كانت ستُنجح أربعة وتُخفق اثنين بلا ما يقول أيّهما.
                                      لكل مُسنَد إليه صفّه، فمن هناك يحمله كلٌّ على حدة.
PATCH  /tasks/{id}                  → تعديل العنوان/الوصف/التاريخ — للصلاحية الكاملة فقط
DELETE /tasks/{id}                  → للصلاحية الكاملة فقط، ويحذف مرفقاتها وملفاتها
PUT    /tasks/{id}/state            → { state, note? } — بديل RPC set_personal_task_state.
                                      باب المُسنَد إليه: صاحب القائمة دائماً، والصلاحية الكاملة كذلك
POST   /tasks/{id}/attachments      → نفس عقد §12.3؛ يكتبها من يجوز له تغيير الحالة
...
3. **الصلاحية الكاملة** (تعديل/حذف/كل شيء): الكاتب؛ والمدير أو حامل
   `tasks.assign` على المهام **المُسندة** فقط. ما كتبه شخص لنفسه دفتره
   الخاص — حتى المدير لا يقرؤه.
4. **القراءة:** صاحب القائمة، والكاتب، وعلى المهام المُسندة فقط: المدير وحامل
   `tasks.assign`.
5. **تغيير الحالة:** صاحب القائمة دائماً (هذا كل حقه على المُسنَد إليه)،
   ومن يملك الصلاحية الكاملة. `note` فارغة ⇒ `null`.
...
**الإشعار:** إدراج مهمة `created_by ≠ profile_id` ⇒ إشعار لصاحبها
(`type: personal_task_assigned`, `task_id`). مهمة الشخص لنفسه لا تُشعِر أحداً.

**طابور الإرسال:** تغيير الحالة (مع مرفقاته) يُكتب واقفاً في الميدان ويقبل
الإرسال المتأخر وإعادة التنفيذ — انظر §27؛ نوع العملية على الجهاز
`personal.task_state` (كان `module.task_state`).

### 25.3 الصلاحية الجديدة

| الرمز | الوصف | يتطلب |
| ----- | ----- | ----- |
| `tasks.assign` | كتابة مهمة على قائمة شخص آخر، ورؤية المهام المُسندة كلها | `employees.view` |

قسم أعلى مستقل `tasks` (ترتيبه 15) — الكود ليس تحت `modules.` عمداً. مُنحت
في الترحيل لكل حاملي `modules.tasks`. وهي **حارس شاشة كاملة**
(`/tasks/manage`) لا زرّاً داخل «مهامي».
...
| جدول `place_codes` | السرّ القابل للتجديد، ومتى جُدِّد وبيد من |
| `place_code(item)` / `rotate_place_code(item)` | القراءة والتجديد. **بلا مهلة سماح** |
